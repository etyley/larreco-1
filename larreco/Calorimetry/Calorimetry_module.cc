...
namespace calo {
  ...
  private:
    ...
    bool fUseArea;
    bool fSCE, fSCEPos, fSCEDir, fSCEEField;
      ...
      for (size_t itp = 0; itp < tracklist[trkIter]->NumberTrajectoryPoints(); ++itp){
        ...
        if (tpcid.isValid) {
          try{
            ...
            if(sce->EnableCalSpatialSCE()&&fSCEPos) posOffsets = sce->GetCalPosOffsets(geo::Point_t(pos),tpcid.TPC);
            if(sce->EnableCalSpatialSCE()&&fSCEDir) dirOffsets = sce->GetCalPosOffsets(geo::Point_t{pos.X() + fTrkPitch*dir.X(), pos.Y() + fTrkPitch*dir.Y(), pos.Z() + fTrkPitch*dir.Z()},tpcid.TPC);
          ...
          for (size_t ii = 0; ii<vhit.size(); ++ii){
            if (vhit[ii].key() == allHits[hits[ipl][ihit]].key()){
             ...
             //Correct location for SCE
              geo::Point_t loc = tracklist[trkIter]->LocationAtPoint(vmeta[ii]->Index());
              geo::Vector_t locOffsets = {0., 0., 0.,};
              if(sce->EnableCalSpatialSCE()&&fSCEPos) locOffsets = sce->GetCalPosOffsets(loc,vhit[ii]->WireID().TPC);
...
//    xyz = isp->second->XYZ();
  ...
  if (kx*kx+ky*ky+kz*kz){
    ...
    geo::Vector_t posOffsets = {0., 0., 0.};
    geo::Vector_t dirOffsets = {0., 0., 0.};
    if(sce->EnableCalSpatialSCE()&&fSCEPos) posOffsets = sce->GetCalPosOffsets(geo::Point_t{xyz3d[0], xyz3d[1], xyz3d[2]},hit->WireID().TPC);
    if(sce->EnableCalSpatialSCE()&&fSCEDir) dirOffsets = sce->GetCalPosOffsets(geo::Point_t{xyz3d[0] + pitch*kx, xyz3d[1] + pitch*ky, xyz3d[2] + pitch*kz},hit->WireID().TPC);
