...
namespace calo{
  ...
  {
    return dEdx_AMP(hit, pitch, T0, detprop->Efield());
  }
  double CalorimetryAlg::dEdx_AMP(art::Ptr< recob::Hit >  hit, double pitch, double T0, double Efield) const
  {
    return dEdx_AMP(hit->PeakAmplitude()/pitch, hit->PeakTime(), hit->WireID().Plane, T0, Efield);
  }
  ...
  double CalorimetryAlg::dEdx_AMP(recob::Hit const&  hit, double pitch, double T0, double Efield) const
  {
    return dEdx_AMP(hit.PeakAmplitude()/pitch, hit.PeakTime(), hit.WireID().Plane, T0, Efield);
  }
  ...
  {
    return dEdx_AMP(dQ, time, pitch, plane, T0, detprop->Efield());
  }
  double CalorimetryAlg::dEdx_AMP(double dQ, double time, double pitch, unsigned int plane, double T0, double Efield) const
  {
    double dQdx   = dQ/pitch;           // in ADC/cm
    return dEdx_AMP(dQdx, time, plane, T0, Efield);
  }

  // ----------------------------------------------------------------------------------//
  double CalorimetryAlg::dEdx_AMP(double dQdx,double time, unsigned int plane, double T0) const
  {
    return dEdx_AMP(dQdx, time, plane, T0, detprop->Efield());
  }
  double CalorimetryAlg::dEdx_AMP(double dQdx,double time, unsigned int plane, double T0, double Efield) const
  {
    double fADCtoEl=1.;
  ...
  {
    return dEdx_AREA(hit, pitch, T0, detprop->Efield());
  }
  ...
  {
    return dEdx_AREA(dQ, time, pitch, plane, T0, detprop->Efield());
  }
  ...
  {
    return dEdx_AREA(dQdx, time, plane, T0, detprop->Efield());
  }
  double CalorimetryAlg::dEdx_AREA(double dQdx,double time, unsigned int plane, double T0, double Efield) const
  {
    double fADCtoEl=1.;
  ...
  {
    return dEdx_from_dQdx_e(dQdx_e, time, T0, detprop->Efield());
  }
  double CalorimetryAlg::dEdx_from_dQdx_e(double dQdx_e, double time, double T0, double Efield) const
  {
    ...
    if(fUseModBox) {
      return detprop->ModBoxCorrection(dQdx_e, Efield);
    } else {
      return detprop->BirksCorrection(dQdx_e, Efield);
    }
  }
